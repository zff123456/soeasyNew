<template>
  <div class="sixkaoshi" style="margin-top:70px;">
    <div class="centers">
       <div class="title-box">
      ...
      <div class="kaoshi-box">
        <div class="button-box">
           ...
           <el-button type="warning" plain @click.native="check(2)" :class="isCheck == 2?'checked':''">听篇章</el-button>
           <el-button type="warning" plain @click.native="check(3)" :class="isCheck == 3?'checked':''">选词填空</el-button>
           <el-button type="warning" plain @click.native="check(4)" :class="isCheck == 4?'checked':''">听讲座对话</el-button>
      ...
      <div class="datika" ref="product">
        <div class="inner-box">
          ...
          <div class="one-box">
            ...
            <div class="span-box">
              <span :class="zuowen.yn == ''? '': 'chec'"  @click ="check(1)">1</span>
              <!-- v-if="is_tijiao == false" -->
          ...
          <div class="one-box">
            ...
            <div class="span-box">
              <span :class="xuancitiankongDy[key] == null? '': 'chec'" v-for="(item, key, index) in xuancitiankong.xuan" @click ="check(3)">{{key + 1}}</span>
          ...
          <div class="one-box">
            ...
            <div class="span-box" v-for="(item, key, index) in xinwenbaodao">
              <em style="font-style:normal;margin-right:5px;font-size:16px">第{{ key + 1 }}大题</em>
              <span :class="ie.yn == ''? '': 'chec'" v-for="(ie, k) in item.xuanzhe[0]"  @click ="check(4)">{{k + 1}}</span>
          ...
          <div class="one-box">
            ...
            <div class="span-box">
              <span :class="item.yn == ''? '': 'chec'" v-for="(item, key, index) in changyuedu.xuanzhe"  @click ="check(5)">{{key + 1}}</span>
          ...
          <div class="one-box">
            ...
            <div class="span-box" v-for="(item, key, index) in zixiyuedu">
              <em style="font-style:normal;margin-right:5px;font-size:16px;">第{{ key + 1 }}大题</em>
              <span :class="ie.yn == ''? '': 'chec'" v-for="(ie, k) in item.xuanzhe[0]"  @click ="check(6)">{{k + 1}}</span>
          ...
          <div class="one-box">
            ...
            <div class="span-box">
              <span :class="pianzhangfanyi.yn == ''? '': 'chec'"  @click ="check(8)">1</span>
      ...
      <div class="component-box">
        ...
        <div class="big-box" v-show="isCheck == 1">
          ...
          <div v-html="zuowen.daan" style="border: 1px solid #ccc;padding:10px;font-size:14px;line-height:25px;">{{zuowen.daan}}</div>
          <div v-html="zuowen.jiexi" style="margin-top:10px;border: 1px solid #ccc;padding:10px;font-size:14px;line-height:25px;">{{zuowen.jiexi}}</div>
        </div>
        <!-- 听篇章 -->
        <div class="big-box" v-show="isCheck == 2">
          ...
          <div v-for="(item, key, index) in duanwenlijie" class="duanwen-box">
            ...
            <div  v-html="item.yiwen" style="margin-top:10px;border: 1px solid #ccc;padding:10px;font-size:14px;line-height:25px;" >{{item.yiwen}}</div>
            <div class="duanwen-options">
              <div class="option-one" v-for="(ie, k, index) in item.xuanzhe[0]">
                ...
                <el-popover
                  ...
                  <el-button slot="reference" type="warning" style="float:right;margin-top:5px" >题眼</el-button>
                </el-popover>
                <div class="xuanxiang" v-for="(i,e) in ie.xuanxiang" style="margin-top: 5px">
                  <el-radio v-model="ie.yn" :label="e" :class="{ red:e==ie.daan}">{{i}}</el-radio>
                </div>
                <!-- <div
                  v-if="is_tijiao"
                  v-html="ie.tiyan"
                ...
                <div  v-if="is_tijiao"
                  v-html="ie.jiexi"
                  style="margin-top:10px;border: 1px dashed #ccc;padding:10px;font-size:14px;line-height:25px;"
                >{{ie.jiexi}}</div>
              </div>
            </div>
          </div>
        ...
        <div class="big-box" v-show="isCheck == 3">
          <div class="xuancutiankong-box" style="overflow:hidden">
            <div
              ...
              style="padding:10px;padding:10px;font-size:14px;line-height:25px;"
          ...
          <div style="padding:10px;padding:10px;font-size:14px;line-height:25px;font-size:14px;line-height:25px;" >{{xuancitiankong.daan}}</div>
          <div style="padding:10px;padding:10px;font-size:14px;line-height:25px;font-size:14px;line-height:25px;" >{{xuancitiankong.yiwen}}</div>
        </div>
        <!-- 听讲座对话 -->
        <div class="big-box" v-show="isCheck == 4">
          ...
          <div v-for="(item, key, index) in xinwenbaodao" class="duanwen-box">
            <el-button
              ...
              style="margin-top:5px; background:#00EAEE;border:none;font-size:14px;line-height:25px;"
            >第{{key + 1}}题</el-button>
            <audio ref="xinwenAudio"></audio>
            <div
              v-if="is_tijiao"
              v-html="item.yiwen"
              style="margin-top:10px;border: 1px solid #ccc;padding:10px;font-size:14px;line-height:25px;"
            >{{item.yiwen}}</div>
            <div class="duanwen-options">
              <div class="option-one" v-for="(ie, k, index) in item.xuanzhe[0]">
          ...
          <div>
            ...
            <div v-for="(item, key, index) in changyuedu.xuanzhe">
              <el-popover
                ...
                trigger="hover">
                <template slot-scope="scope">
                  <div v-html="item.tiyan" style="border: 1px dashed #ccc;padding:10px;max-height:70vh;overflow: auto;font-size:14px;line-height:25px;"></div>
                </template>
              ...
              <div
                style="margin-top:10px;padding:10px;margin-left:10px;font-size:14px;line-height:25px;"
                v-if="is_tijiao"
              >答案：{{item.daan}}</div>
              <!-- <div v-html="item.tiyan" style="padding:10px;margin-left:10px;" v-if="is_tijiao"></div> -->
              <div v-html="item.jiexi" style="padding:10px;margin-left:10px;font-size:14px;line-height:25px;"></div>
            </div>
          </div>
        </div>
        <!-- 仔细阅读 -->
        <div class="big-box" v-show="isCheck == 6">
          ...
          <div v-for="(item, key, index) in zixiyuedu" class="duanwen-box">
            <div
              ...
              style="margin-top:10px;padding:10px;font-size:14px;line-height:25px;"
            >{{item.wenzhang}}</div>
            <div class="duanwen-options">
              <div class="option-one" v-for="(ie, k, index) in item.xuanzhe[0]">
                <el-popover
                  ...
                  <el-button slot="reference" type="warning" style="float:right" >题眼</el-button>
                </el-popover>
        ...
        <div class="big-box" v-show="isCheck == 7">
          ...
          <div v-for="(item, key, index) in changduihua" class="duanwen-box">
            <el-button size="mini" type="primary" icon="el-icon-video-play" @click ="play_chang(item,key)" :disabled="duanwenPlay" style="margin-top:5px; background:#00EAEE;border:none">第{{key + 1}}题</el-button>
            ...
            <div class="duanwen-options">
              <div class="option-one" v-for="(ie, k, index) in item.xuanzhe[0]">
                ...
                <!-- <div
                  ...
                  style="margin-top:10px;border: 1px dashed #ccc;padding:10px"
                >{{ie.tiyan}}</div> -->
                <div v-html="ie.jiexi" style="margin-top:10px;border: 1px dashed #ccc;padding:10px;font-size:14px;line-height:25px;">{{ie.jiexi}}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="big-box" v-show="isCheck == 8">
          ...
          <el-input v-model="pianzhangfanyi.yn" :autosize="{ minRows: 24}" type="textarea"  style="border-radius: 0;margin: 10px 0;font-size:14px;line-height:25px;" ></el-input>
          <div  v-html="pianzhangfanyi.daan"  style="border: 1px solid #ccc;padding:10px;font-size:14px;line-height:25px;" >{{pianzhangfanyi.daan}}</div>
          <div v-html="pianzhangfanyi.jiexi"  style="margin-top:10px;border: 1px solid #ccc;padding:10px;font-size:14px;line-height:25px;" >{{pianzhangfanyi.jiexi}}</div>
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    async gettItem() {
      ...
      const res = await this.$http.post(
        "http://ruanjian.chinadingao.com/admin/Examination/paperlst",
        qs.stringify({id: 200,u_id:1,identity: 1})
      );
