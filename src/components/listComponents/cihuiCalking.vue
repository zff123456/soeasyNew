  ...
  <div class="right" v-loading="isLoading" element-loading-text="拼命加载中">
    <div class="table-box">
      <el-table style="width: 100%" border="" stripe :data="flatten(tableData.slice((currentPage -1) * pagesize, currentPage * pagesize))" :row-style="{height:'48px'}"  :header-row-style="{height:'48px'}">
        ...
        <el-table-column prop="wenti" label="问题" min-width="160"></el-table-column>
        <el-table-column prop="daan" label="答案" width="160"></el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    ...
    async getThesisList() {
      this.isLoading = true;
      const res = await this.$http.post("http://ruanjian.chinadingao.com/admin/paper/abcihuitiankonglst",qs.stringify({cet: this.paperId }));
