<template>
  <div class="right" v-loading="isLoading" element-loading-text="拼命加载中">
    ...
    <div class="dialog-box">
      <el-dialog title="问题列表" :visible.sync="dialogTableVisible" width="60%">
...
export default {
  ...
  methods: {
    ...
    async lookChecket(i){ // 查看选项
      ...
      this.isLoading = true;
      const res = await this.$http.post("http://ruanjian.chinadingao.com/admin/paper/CETETzixiyueduxuanxiang",qs.stringify({id: i.id}));
      console.log(res);
      if (res.status === 200 && res.data.code === "1") {
