  ...
  <div class="right" v-loading="isLoading" element-loading-text="拼命加载中">
    <div class="table-box">
      <el-table style="width: 100%" border="" stripe :data="flatten(tableData.slice((currentPage -1) * pagesize, currentPage * pagesize))" :row-style="{height:'48px'}"  :header-row-style="{height:'48px'}">
        ...
        <el-table-column prop="test_paper_type_name" label="操作">
    ...
    <el-dialog title="问题"   :visible.sync="dialogVisible"   width="50%">
         <div  v-html="lookData.wenzhang"   style="padding:10px;box-sizing:border-box;margin:5px 0"></div>
         <div  v-html="lookData.cankaodaan"   style="padding:10px;box-sizing:border-box;margin:10px 0"></div>
         <div  v-html="lookData.jiexi"   style="padding:10px;box-sizing:border-box;margin:10px 0"></div>
    </el-dialog>
...
export default {
  ...
  methods: {
    look(val){
      ...
      this.lookData.wenzhang=val.wenzhang
    ...
    async getThesisList() {
      this.isLoading = true;
      const res = await this.$http.post("http://ruanjian.chinadingao.com/admin/paper/yingyihanlst",qs.stringify({cet: this.paperId , type: 1}));
