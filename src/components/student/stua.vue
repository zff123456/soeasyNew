<template>
  <div class="tiku"  >
    <!-- v-loading="isLoading" element-loading-text="拼命加载中" -->
     ...
     <div class="centers">
      <div class="header" style="padding-top:10px;">
        <el-button type="warning"  plain v-for="(item,key) in aList" :key="key" @click="cheched(3,item)" style="border:none"> {{item}} </el-button>
    </div>
      <div class="content" style="background-color: #ffffff;" >
          <div class="content-box" v-if="problem == '作文' || problem == '英译汉' || problem == '篇章翻译'">
              <div class="big-box">
                <div class="zuowen-title" v-if="problem != '英译汉'" style="font-size: 14px;line-height: 25px;">{{itemOne.wenti}}</div>
                ...
                <el-input v-model="itemOne.yn" :autosize="{ minRows: 26}" type="textarea" style="font-size: 14px;line-height: 25px;border-radius: 0;margin: 10px 0;font-size: 14px;line-height: 25px;"></el-input>
                <div v-html="itemOne.daan" v-if="itemOne.state == 0 && problem != '英译汉'" style="font-size: 14px;line-height: 25px;border: 1px solid #ccc;padding:10px">{{itemOne.daan}}</div>
                <div v-html="itemOne.cankaodaan" v-if="itemOne.state == 0 && problem == '英译汉'" style="font-size: 14px;line-height: 25px;border: 1px solid #ccc;padding:10px"></div>
                <div v-html="itemOne.jiexi" v-if="itemOne.state == 0" style="font-size: 14px;line-height: 25px;margin:10px 0;border: 1px solid #ccc;padding:10px">{{itemOne.jiexi}}</div>
                <!-- 提交按钮。 -->
   ...
   <div class="content-box" v-if="problem == '新闻报道' || problem == '听讲座对话'|| problem == '听长对话' || problem == '听短文理解' || problem == '听篇章' || problem == '听短对话'">
      <div class="big-box" v-for="(item,key) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
          ...
          <el-button size="mini" type="primary" icon="el-icon-video-play" @click="play_xinwen(item,key)" style="padding: 6px 11px;font-size: 12px;margin:10px 0;background: rgb(0, 234, 238); border: none;" >播放大题听力</el-button>
          <div v-if="item.state == 0" v-html="item.yiwen" style="font-size: 14px;line-height: 25px;margin-top:10px;border: 1px solid #ccc;padding:10px">{{item.yiwen}}</div>
          <div class="duanwen-options" >
            <div class="option-one" v-for="(ie, k, index) in item.xuanzhe[0]">
              ...
              <div class="xuanxiang" v-for="(i,e) in ie.xuanxiang" style="margin-top: 5px">
                <el-radio v-model="ie.yn" :label="e" :class="item.state == 0 && ie.yn != ie.daan && e == ie.daan? 'red':''" style="font-size: 14px;line-height: 25px;">{{i}}</el-radio>
              </div>
              <div v-if="item.state == 0" v-html="ie.tiyan" style="font-size: 14px;line-height: 25px;margin-top:10px;border: 1px dashed #ccc;padding:10px">{{ie.tiyan}}</div>
              <div v-if="item.state == 0" v-html="ie.jiexi" style="font-size: 14px;line-height: 25px;margin-top:10px;border: 1px dashed #ccc;padding:10px">{{ie.jiexi}}</div>
            </div>
          </div>
  ...
  <div class="content-box" v-if="problem == '听单句'">
      <div class="big-box" v-for="(item,key) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
          ...
          <!-- <audio ref="xinwenOpt" size="mini"></audio> -->
            <div class="duanwen-options" >
              <div class="option-one" v-for="(ie, k, index) in item.xuanzhe[0]">
                <el-button type="success" icon="el-icon-video-play" @click="play_xinwen_opt(ie,key,k)" ref="dw_btn" style="padding: 5px 10px;font-size: 12px;margin:10px 0;background: rgb(0, 234, 238); border: none;">第({{k + 1}})小题听力</el-button>
                <div class="xuanxiang" v-for="(i,e) in ie.xuanxiang" style="font-size: 14px;line-height: 25px;margin-top: 5px">
  ...
  <div class="content-box" v-if="problem == '选词填空'">
      <div class="big-box" v-for="(item,key) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
        <div class="xuancutiankong-box" style="overflow:hidden">
          <div v-html="item.duanwen" style="font-size: 14px;line-height: 25px;padding:10px;padding:10px;">{{item.duanwen}}</div>
        ...
        <div style="font-size: 14px;line-height: 25px;padding:10px;padding:10px;" v-if="item.state == 0">{{item.daan}}</div>
        <div style="font-size: 14px;line-height: 25px;padding:10px;padding:10px;" v-if="item.state == 0">{{item.yiwen}}</div>
        <!-- 提交按钮。 -->
  ...
  <div class="content-box" v-if="problem == '阅读理解' || problem == '词汇匹配'">
      <div class="big-box" v-for="(item,key) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
        <div v-html="item.duanwen" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px" v-if="problem == '阅读理解'"></div>
        <div v-html="item.beixuanxiang" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px" v-if="problem == '词汇匹配'"></div>
        <div v-html="item.jiexi" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px" v-if="problem == '词汇匹配' && item.state == 0"></div>
        <div v-if="item.state == 0" v-html="item.cankaodaan" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px"></div>
        <div class="xuanxiang" v-for="(ie, k, index) in item.xuanzhe">
          <div v-html="ie.wenti" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px"></div>
          <el-input v-model="ie.yn" placeholder="请输入答案" style="font-size: 14px;line-height: 25px;box-sizing: border-box;padding: 0 10px;margin-bottom:10px"></el-input>
          <div class="zuowen-title" v-if="item.state == 0">{{ie.daan}}</div>
          <div v-if="item.state == 0 && problem == '阅读理解'" v-html="ie.tiyan" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px"></div>
        </div>
        <!-- 提交按钮。 -->
        <el-button type="primary" @click="submitXinwen(item)">提交</el-button>
             <!-- 分页插件。 -->
  ...
  <div class="content-box" v-if="problem == '长篇阅读'">
      <div class="big-box" v-for="(ie,k) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
        <div v-html="ie.beixuanxiang" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px"></div>
          <div v-for="(item, key, index) in ie.xuanzhe" style="margin-bottom:10px">
            ...
            <div v-html="item.wenti" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px">{{item.wenti}}</div>
            <el-select v-model="item.yn" placeholder="请选择" style="margin-left:20px">
              <el-option v-for="item in english"  :key="item.value"  :label="item.label"  :value="item.value">  </el-option>
            </el-select>
            <div style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px;margin-left:10px;" v-if="ie.state == 0 ">答案：{{item.daan}}</div>
            <div v-html="item.tiyan" style="font-size: 14px;line-height: 25px;padding:10px;margin-left:10px;" v-if="ie.state == 0 "></div>
            <div v-html="item.jiexi" style="font-size: 14px;line-height: 25px;padding:10px;margin-left:10px;" v-if="ie.state == 0 "></div>
          </div>
  ...
  <div class="content-box" v-if="problem == '仔细阅读'">
      <div class="big-box" v-for="(item,key) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
        <div class="zuowen-title" style="font-size: 14px;line-height: 25px;">Directions: In this section, you will hear several newsreports.At the end of each news report, you will hear two or three questions. Both the news report and the questions will be spoken only once. After you hear a question, you must choose the best answer from the four choices marked A，B，C and D. Then select the corresponding letter on the web page.</div>
        <div v-html="item.wenzhang" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px">{{item.wenzhang}}</div>
        <div class="duanwen-options">
          <div class="option-one" v-for="(ie, k, index) in item.xuanzhe[0]">
            <div v-html="ie.wenti" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px">{{ie.wenti}}</div>
            <div class="xuanxiang" v-for="(i,e) in ie.xuanxiang" style="margin-top: 5px">
              <el-radio v-model="ie.yn" :label="e" :class="item.state == 0 && ie.yn != ie.daan && e == ie.daan? 'red':''" >{{i}}</el-radio>
            </div>
            <div v-if="item.state == 0" v-html="ie.jiexi" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px">{{ie.jiexi}}</div>
            <div v-if="item.state == 0" v-html="ie.tiyan" style="font-size: 14px;line-height: 25px;padding:10px">{{ie.tiyan}}</div>
          </div>
        </div>
        <!-- 提交按钮。 -->
        <el-button type="primary" @click="submitXinwen(item)">提交</el-button>
    ...
    <div class="content-box" v-if="problem == '阅读填空' || problem == '短文填空' ">
      <div class="big-box" v-for="(ie,k) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
        ...
        <div style="overflow:hidden">
          <el-button size="mini" type="primary" icon="el-icon-video-play" @click="play_yuedu(ie,k)" v-if="problem == '短文填空'" style="padding: 5px 10px;font-size: 12px;margin:10px 0;background: rgb(0, 234, 238); border: none;">播放音频</el-button>
          <div v-html="ie.wenti" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px" v-if="problem == '短文填空'"></div>
          <div v-html="ie.duanwen" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px" v-if="problem == '阅读填空'"></div>
          <div style="height: 100%;">
            <div v-for="(item, key, index) in ie.yn" >
              <div style="float:left;margin: 10px 10px">
                <span>第{{key + 1}}题：</span>
                <el-input v-model="ie.yn[key]" placeholder="请输入答案" style="font-size: 14px;line-height: 25px;box-sizing: border-box;padding: 0 5px;width: 200px"></el-input>
              </div>
            </div>
          </div>
          <div v-if="ie.state == 0 " v-html="ie.daan" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px"></div>
          <div v-if="ie.state == 0 " v-html="ie.yiwen" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px"></div>
          <div v-if="ie.state == 0 " v-html="ie.cankaodaan" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px"></div>
        </div>
        <!-- 提交按钮。 -->
        <el-button type="primary" @click="submitYuedu(ie)">提交</el-button>
            <!-- 分页插件。 -->
    ...
    <div class="content-box" v-if="problem == '词汇选择' || problem == '翻译选择'">
      <div class="big-box" v-for="(item,key) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
        <div v-html="item.wenti" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px"></div>
    ...
    <div class="content-box" v-if="problem == '词汇填空'">
      <div class="big-box" v-for="(item,key) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
        <div v-html="item.wenti" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px"></div>
        <el-input v-model="item.yn" placeholder="请输入答案" style="font-size: 14px;line-height: 25px;box-sizing: border-box;padding: 0 10px;margin-bottom:10px"></el-input>
        <div v-if="item.state == 0" v-html="item.yiwen" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px"></div>
        <div v-if="item.state == 0" v-html="item.daan" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px"></div>
        <div v-if="item.state == 0" v-html="item.cankaodaan" style="font-size: 14px;line-height: 25px;margin-top:10px;padding:10px"></div>
        <!-- <div style="height:20px;width:110%;background-color: #F8F8F8;margin-bottom:10px;transform: translateX(-30px);"></div> -->
        <!-- 提交按钮。 -->
        <el-button type="primary" @click="submitXinwen(item)">提交</el-button>
...
export default {
  ...
  mounted() {
      this.getTimu(this.aList[0], this.type);
  },
  watch: {
  itemData( val ){
    this.changeOne( val );
  ...
  async getTimu(item, type){
    ...
    this.type = type;
    this.currentPage = 1;
    const res = await this.$http.post("http://ruanjian.chinadingao.com/admin/Examination/danxiangtupo",qs.stringify({ test_paper_type_name:3, problem: this.problem }));
