  ...
  <div class="fourKaoshi" style="padding-top:61px">
    ...
    <div class="kaoshi-box">
      <div class="button-box">
        <el-button type="warning" plain @click="check(1)" :class="isCheck == 1?'checked':''">作文题</el-button>
        <el-button type="warning" plain @click="check(2)" :class="isCheck == 2?'checked':''">听短文理解</el-button>
        <el-button type="warning" plain @click="check(3)" :class="isCheck == 3?'checked':''">选词填空</el-button>
        <el-button type="warning" plain @click="check(4)" :class="isCheck == 4?'checked':''">新闻报道</el-button>
        <el-button type="warning" plain @click="check(5)" :class="isCheck == 5?'checked':''">长篇阅读</el-button>
        <el-button type="warning" plain @click="check(6)" :class="isCheck == 6?'checked':''">仔细阅读</el-button>
        <el-button type="warning" plain @click="check(7)" :class="isCheck == 7?'checked':''">听长对话</el-button>
        <el-button type="warning" plain @click="check(8)" :class="isCheck == 8?'checked':''">篇章翻译</el-button>
        ...
        <el-button type="warning"  v-if="identity != 2 && isChakan != 1" plain @click="isdialogVisible = true" style="float:right;background-color: #E6A23C;color:#fff;border-radius: 4px;margin-right:10px" >暂停作答</el-button>
      </div>
      <!-- 答题卡 -->
      <div class="datika">
...
export default {
  ...
  created() {
    ...
    this.kaoshiId = localStorage.getItem("kaoshiId");   //试卷的id
    this.school_id = localStorage.getItem("school_id");  //学校的id
    this.identity = localStorage.getItem("identity");   //身份
    this.id= localStorage.getItem("id");   //教师或者学生的id
