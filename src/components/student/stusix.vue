    ...
    <!-- v-loading="isLoading" element-loading-text="拼命加载中" -->
     ...
     <div class="centers">
      <div class="header" style="padding-top:10px;">
        <el-button type="warning"  plain v-for="(item,key) in sixList" :key="key" style="border:none" @click="cheched(2,item)" > {{item}} </el-button>
    </div>
  ...
  <div class="content-box" v-if="problem == '仔细阅读'">
        ...
        <!-- <audio ref="xinwenOpt" size="mini"></audio> -->
          ...
          <div class="duanwen-options" style="margin-top:10px">
            <div class="option-one" v-for="(ie, k, index) in item.xuanzhe">
              ...
              <div v-html="ie.wenti" style="font-size:14px;line-height: 25px;padding:10px"></div>
              <el-input v-model="ie.yn" placeholder="请输入答案" style="box-sizing: border-box;padding: 0 10px"></el-input>
              <div v-if="item.state == 0" v-html="ie.jiexi" style="font-size:14px;line-height: 25px;margin-top:10px;border: 1px dashed #ccc;padding:10px">{{ie.tiyan}}</div>
              <div v-if="item.state == 0" v-html="ie.daan" style="font-size:14px;line-height: 25px;margin-top:10px;border: 1px dashed #ccc;padding:10px">{{ie.jiexi}}</div>
            </div>
          </div>
        <!-- <div style="height:20px;width:110%;background-color: #F8F8F8;margin:10px 0;transform: translateX(-30px);"></div> -->
        <!-- 提交按钮。 -->
    ...
    <div class="content-box" v-if="problem == '词汇选择' || problem == '翻译选择'">
      <div class="big-box" v-for="(item,key) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
        ...
        <div v-if="item.state == 0" v-html="item.jiexi" style="font-size:14px;line-height: 25px;margin-top:10px;padding:10px"></div>
        <div style="height:20px;width:110%;background-color: #F8F8F8;margin-bottom:10px;transform: translateX(-30px);"></div>
        <!-- 提交按钮。 -->
...
export default {
  ...
  mounted() {
      // this.getList()
      this.getTimu(this.sixList[0], this.type);
  },
  watch: {
  itemData( val ){
    this.changeOne( val );
  ...
  async getTimu(item, type){
    ...
    this.type = type;
    this.currentPage = 1;
    const res = await this.$http.post("http://ruanjian.chinadingao.com/admin/Examination/danxiangtupo",qs.stringify({ test_paper_type_name:2, problem: this.problem }));
