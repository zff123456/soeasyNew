<template>
    ...
    <!-- v-loading="isLoading" element-loading-text="拼命加载中" -->
     ...
     <div class="centers">
      <div class="header" style="padding-top:10px;">
        <el-button type="warning"  plain v-for="(item,key) in bList" :key="key" @click="cheched(4,item)" style="border:none"> {{item}} </el-button>
    </div>
      <div class="content" style="background-color: #ffffff;" >
          <div class="content-box" v-if="problem == '作文' || problem == '英译汉' || problem == '篇章翻译'">
              <div class="big-box">
                ...
                <div v-html="itemOne.wenzhang" v-if="problem == '英译汉'" style="font-size: 14px;line-height: 25px;"></div>
                ...
                <el-input v-model="itemOne.yn" :autosize="{ minRows: 26}" type="textarea" style="font-size: 14px;line-height: 25px;border-radius: 0;margin: 10px 0"></el-input>
  ...
  <div class="content-box" v-if="problem == '听单句'">
      <div class="big-box" v-for="(item,key) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
          ...
          <!-- <audio ref="xinwenOpt" size="mini"></audio> -->
            <div class="duanwen-options" >
              <div class="option-one" v-for="(ie, k, index) in item.xuanzhe[0]">
                <el-button type="success" icon="el-icon-video-play" @click="play_xinwen_opt(ie,key,k)" ref="dw_btn" style="padding: 5px 10px;font-size: 12px;margin:10px 0 ;background: rgb(174, 93, 161); border: none;">第({{k + 1}})小题听力</el-button>
                <div class="xuanxiang" v-for="(i,e) in ie.xuanxiang" style="margin-top: 5px">
                  <el-radio v-model="ie.yn" :label="e" :class="item.state == 0 && ie.yn != ie.daan && e == ie.daan? 'red':''">{{i}}</el-radio>
                </div>
                <div v-if="item.state == 0" v-html="ie.tiyan" style="font-size: 14px;line-height: 25px;margin-top:10px;border: 1px dashed #ccc;padding:10px">{{ie.tiyan}}</div>
  ...
  <div class="content-box" v-if="problem == '阅读理解' || problem == '词汇匹配'">
      <div class="big-box" v-for="(item,key) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
        <div v-html="item.duanwen" style="font-size: 14px;line-height: 25px;padding:10px" v-if="problem == '阅读理解'"></div>
        <div v-html="item.beixuanxiang" style="font-size: 14px;line-height: 25px;padding:10px" v-if="problem == '词汇匹配'"></div>
        ...
        <div class="xuanxiang" v-for="(ie, k, index) in item.xuanzhe">
          ...
          <div class="zuowen-title" v-if="item.state == 0" style="font-size: 14px;line-height: 25px;" >{{ie.daan}}</div>
  ...
  <div class="content-box" v-if="problem == '仔细阅读'">
    ...
    <div class="content-box" v-if="problem == '阅读填空' || problem == '短文填空' ">
      <div class="big-box" v-for="(ie,k) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
        ...
        <div style="overflow:hidden">
          <el-button size="mini" type="primary" icon="el-icon-video-play" @click="play_yuedu(ie,k)" v-if="problem == '短文填空'" style="padding: 6px 11px;font-size: 12px;margin:10px 0;background: rgb(0, 234, 238); border: none;">播放音频</el-button>
    ...
    <div class="content-box" v-if="problem == '听短文' ">
      <div class="big-box" v-for="(item,key) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
        ...
        <!-- <audio ref="xinwenOpt" size="mini"></audio> -->
          ...
          <div v-if="item.state == 0" v-html="item.yiwen" style="margin-top:10px;border: 1px solid #ccc;padding:10px;font-size: 14px;line-height: 25px;">{{item.yiwen}}</div>
          <div class="duanwen-options" style="margin-top:10px">
            <div class="option-one" v-for="(ie, k, index) in item.xuanzhe">
              ...
              <el-input v-model="ie.yn" placeholder="请输入答案" style="box-sizing: border-box;padding: 0 10px;font-size: 14px;line-height: 25px;"></el-input>
    ...
    <div class="content-box" v-if="problem == '词汇选择' || problem == '翻译选择'">
      <div class="big-box" v-for="(item,key) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
        <div v-html="item.wenti" style="padding:10px;font-size: 14px;line-height: 25px;"></div>
    ...
    <div class="content-box" v-if="problem == '词汇填空'">
      <div class="big-box" v-for="(item,key) in itemData.slice((currentPage -1) * pagesize, currentPage * pagesize)">
        <div v-html="item.wenti" style="padding:10px;font-size: 14px;line-height: 25px;"></div>
        <el-input v-model="item.yn" placeholder="请输入答案" style="box-sizing: border-box;padding: 0 10px;margin-bottom:10px;font-size: 14px;line-height: 25px;"></el-input>
        <div v-if="item.state == 0" v-html="item.yiwen" style="margin-top:10px;padding:10px;font-size: 14px;line-height: 25px;"></div>
...
export default {
  ...
  mounted() {
      // this.getList()
      this.getTimu(this.bList[0], this.type);
  },
  watch: {
  itemData( val ){
    this.changeOne( val );
  ...
  async getTimu(item, type){
    ...
    this.type = type;
    this.currentPage = 1;
    const res = await this.$http.post("http://ruanjian.chinadingao.com/admin/Examination/danxiangtupo",qs.stringify({ test_paper_type_name:4, problem: this.problem }));
